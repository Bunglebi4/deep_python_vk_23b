...
class LRUCache:
    ...
    def get(self, key):
        ...
        logging.debug(f"Key '{key}' found in cache. Value: {self.cache[key]}")
        return self.cache[key]

    def set(self, key, value) -> None:
        if key in self.cache:
            logging.debug(f"Key '{key}' already exists. Updating value.")
            self.order.remove(key)
            del self.cache[key]
        elif len(self.cache) >= self.capacity:
            evicted_key = self.order.pop(0)
            logging.debug(f"Evicting key '{evicted_key}' to make space.")
            del self.cache[evicted_key]
        logging.debug(f"Setting key '{key}' with value '{value}' in cache.")
